...
class SegmentationMetric(object):
    ...
    def genConfusionMatrix(self, imgPredict, imgLabel, ignore_labels):  #
        ...
        count = torch.bincount(label.type(torch.LongTensor), minlength=self.numClass ** 2).type(torch.FloatTensor)
    ...
    def accum_calc(self, imgPredict, imgLabel, imnore_labels):
        self.addBatch(imgPredict, imgLabel, imnore_labels)
        pa = self.pixelAccuracy()
        cpa = self.classPixelAccuracy()
        mpa = self.meanPixelAccuracy()
        IoU = self.IntersectionOverUnion()
        mIoU = self.meanIntersectionOverUnion()
        return pa, cpa, mpa, IoU, mIoU


def get_metric_value(seg_metric_obj):
    pa = seg_metric_obj.pixelAccuracy()
    cpa = seg_metric_obj.classPixelAccuracy()
    mpa = seg_metric_obj.meanPixelAccuracy()
    IoU = seg_metric_obj.IntersectionOverUnion()
    mIoU = seg_metric_obj.meanIntersectionOverUnion()
    return pa, cpa, mpa, IoU, mIoU
